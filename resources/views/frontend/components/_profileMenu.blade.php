<div class="relative inline-flex" x-data="{ open: false }" x-cloak>
  @if(Auth::check())
  <button class="w-6 h-6 flex items-center justify-center bg-transparent hover:bg-slate-200 transition duration-150 rounded-full" aria-haspopup="true" @click.prevent="open = !open" :aria-expanded="open">
    ...
    <div class="flex flex-col w-full">
      ...
      <ul class="">
        <li class="border-b last:border-0">
          <a class="block py-2 px-4 hover:bg-[#42474b] text-slate-600 hover:text-white" href="{{ url('/') }}">
        ...
        <li class="last:border-0">
          <a class="block py-1.5 px-4  hover:text-white text-slate-600  hover:bg-[#42474b]" href="{{ url('/') }}">
        ...
        <li class="w-full border-b last:border-0">
          <div class="flex w-full items-center space-x-2 py-1.5 px-2 ">
            <form method="POST" action="{{ route('logout') }}" class="w-full">
              ...
              <a href="{{ route('logout') }}" @click.prevent="$root.submit();" @focus="open = true" @focusout="open = false" class="flex w-full shadow-menu items-center justify-center rounded px-2 py-1.5 space-x-2 bg-[#e31c2d] opacity-90 hover:opacity-90">
