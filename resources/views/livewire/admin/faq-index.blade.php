<section class="container mx-auto  font-mono">
    <div class="w-full flex mb-4 p-2 items-center justify-between">
        <h3 class="text-gray-700 text-3xl font-medium">Faqs</h3>
        <x-jet-button wire:click="showCreateModal">Create Faqs</x-jet-button>
    </div>
                ...
                <tbody class="bg-white">
                    ...
                    <tr class="text-gray-700">
                        <td class="px-4 py-3 border">
                            {{ substr($faq->question, 0, 120) }}...
                        </td>
                        <td class="px-4 py-3 border">
                            {{ substr($faq->answer, 0, 120) }}...
                        </td>
                        ...
                        <td class="px-4 py-3 text-ms font-semibold border">{{ $faq->created_at->format('d-m-Y') }}</td>

                        <td class="px-4 py-3 text-sm border">
                            <x-m-button wire:click="showEditModal({{ $faq->id }})" class="bg-green-500 hover:bg-green-700 text-white">Edit</x-m-button>
                            <x-m-button wire:click="deleteFaq({{ $faq->id }})" class="bg-red-500 hover:bg-red-700 text-white">Delete</x-m-button>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
    ...
    <x-jet-dialog-modal wire:model="showFaqModal">
        @if ($faqId)
        <x-slot name="title">Update Faq</x-slot>
        @else
                            ...
                            <div class="px-4 py-5 bg-white sm:p-6 grid gap-4">
                                <div class="">
                                    <div class="col-span-6 sm:col-span-3">
                                        ...
                                        <textarea wire:model="question" autocomplete="given-name" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" ></textarea>
                                    </div>
                                </div>
                                <div class="">
                                    <div class="col-span-6 sm:col-span-3">
                                        ...
                                        <textarea wire:model="answer" autocomplete="given-name" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" ></textarea>
                                    </div>
                                </div>
                                <div class="">
                                    <div class="col-span-6 sm:col-span-3">
        ...
        <x-slot name="footer">
            <x-m-button wire:click="closeFaqModal" class="bg-gray-600 hover:bg-gray-800 text-white mr-2">Cancel</x-m-button>
            @if ($faqId)
            <x-m-button wire:click="updateFaq">Update</x-m-button>
            @else
            <x-m-button wire:click="createFaq">Create</x-m-button>
            @endif
        </x-slot>
    </x-jet-dialog-modal>
