...
<div class="flex items-center justify-center mt-12 mb-10 space-x-2">
    ...
    <div class="border border-[#E4E4E4] bg-white rounded p-1 text-[#A3A3A3] " aria-label="go to previous page">
    ...
    @else
    <a href="{{ $paginator->previousPageUrl() }}" class="border border-[#E4E4E4] bg-white rounded p-1 text-[#191919] cursor-pointer" aria-label="go to previous page">
        <svg fill="currentColor" width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <path
                fill-rule="evenodd"
                clip-rule="evenodd"
    ...
    {{-- Array Of Links --}}
        @if (is_array($element))
            @foreach ($element as $page => $url)
                ...
                <a 
                    class="rounded p-1 w-8 h-8 text-center text-sm font-medium flex justify-center items-center text-white bg-orange-500" 
                >{{ $page }}
                </a>
                @else 
                <a 
                    class="rounded p-1 w-8 h-8 text-center text-sm font-medium flex justify-center items-center text-gray-900 bg-white border border-[#E4E4E4]" 
                    href="{{ $url }}"
                >{{ $page }}
                </a>
                @endif
    ...
    <a href="{{ $paginator->nextPageUrl() }}" class="bg-white border border-[#E4E4E4] rounded p-1 text-[#191919] cursor-pointer" aria-label="go to next page">
    ...
    @else
    <div class="bg-white border border-[#E4E4E4] rounded p-1 text-[#A3A3A3]" aria-label="go to next page">
        <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
            <path
                fill-rule="evenodd"
                clip-rule="evenodd"
